...
XRAY_RESTARTS=0
MAX_XRAY_RESTARTS=3
SHUTTING_DOWN_FLAG="${LOG_DIR}/shutting-down"
rm -f "$SHUTTING_DOWN_FLAG"
supervise_xray() {
    while :; do
        wait "$XRAY_PID" 2>/dev/null || true
        # If we're shutting down (handover/cleanup), don't restart
        if [[ -f "${SHUTTING_DOWN_FLAG:-}" ]]; then
            exit 0
        fi
...
if [[ -z "${CF_TOKEN:-}" ]]; then
    ...
    start_warp_planes || warn "WARP plane registration failed — WARP tiers will use direct egress"
    (
        export ROTATEWARP2MIN_INTERVAL ROTATEWARP4MIN_INTERVAL ROTATEWARP6MIN_INTERVAL DOMAIN WORKER_URL SEED EXTERNAL_SUB_URLS DEFAULT_EXTERNAL_SUB API_TOKEN SUB_DIR PATH_ROTATE1MIN UUID_ROTATE1MIN PATH_ROTATE2MIN UUID_ROTATE2MIN PATH_ROTATE5MIN UUID_ROTATE5MIN PATH_ROTATE_WARP2MIN UUID_ROTATE_WARP2MIN PATH_ROTATE_WARP4MIN UUID_ROTATE_WARP4MIN PATH_ROTATE_WARP6MIN UUID_ROTATE_WARP6MIN WARP_PORT WARP_PLANE_DIR WARP_PLANE_COUNT WARP_PLANES_ACTIVE WARP_ACTIVE
...
# If the shutdown flag is set, the EXIT trap already ran cleanup — just exit.
if [[ -f "${SHUTTING_DOWN_FLAG:-}" ]]; then
    exit 0
fi
